...
function formatTestResult($result, $index) {
    ...
    if ($result['success']) {
        echo "<p style='color: green;'>[{$timestamp}] {$status} 图片访问 #{$imageId}: 图片可正常访问 ({$fileSizeKB}KB)</p>";
    } else {
        echo "<p style='color: red;'>[{$timestamp}] {$status} 图片访问 #{$imageId}: 访问失败</p>";
    }
    
    // 详细信息
    $detailsJson = json_encode($result, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
    echo "<p style='margin-left: 20px; color: #666; font-size: 12px;'>详细信息: {$detailsJson}</p>";
    echo "<br>";
}
