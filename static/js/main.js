...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // 2025-01-27 修复重复初始化：ImageLoader.init()已在index.php中调用，此处不再重复调用
...
// 回到顶部功能已重构为独立模块，请参考 back-to-top.js
...
function initNotImplementedFeatures() {
    // 功能限制已解除，所有功能现在都可以正常使用
    document.querySelectorAll('.not-implemented').forEach(el => {
        // 移除not-implemented类，恢复正常功能
        el.classList.remove('not-implemented');
    });
}
...
// 注意：DOMContentLoaded事件监听器已在文件开头定义，此处不再重复
// 分类管理相关的事件监听器将在主DOMContentLoaded中初始化

// API基础URL已在auth.js中定义，此处移除重复声明
