...
class MyUploads {
    ...
    async checkLoginStatus() {
        try {
            const response = await fetch(`${this.API_BASE_URL}/auth_unified.php?action=getUserInfo`, {
                credentials: 'include'
            });
    ...
    async logout() {
        try {
            const response = await fetch(`${this.API_BASE_URL}/auth_unified.php?action=logout`, {
                method: 'GET',
                credentials: 'include'
            });
    ...
    async loadWallpapers() {
        try {
            ...
            const response = await fetch(`${this.API_BASE_URL}/my_user_uploads.php?${params}`, {
                credentials: 'include'
            });
            ...
            <tr class="hover:bg-gray-50">
                <td class="px-6 py-4 whitespace-nowrap">
                    <img src="${wallpaper.file_path}" alt="${wallpaper.title}" 
                         ...
                         onclick="myUploads.showDetail(${wallpaper.id})">
                </td>
                <td class="px-6 py-4">
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                    ...
                    ${wallpaper.status === 'rejected' && wallpaper.reject_reason ? `
                        <div class="text-xs text-red-600 mt-1">原因: ${this.escapeHtml(wallpaper.reject_reason)}</div>
                    ` : ''}
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    <div>浏览: ${wallpaper.views}</div>
                    <div>点赞: ${wallpaper.likes}</div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <div class="flex space-x-2">
                        <button onclick="myUploads.showDetail(${wallpaper.id})" 
                                class="text-blue-600 hover:text-blue-900">查看</button>
                        ${wallpaper.status === 'pending' || wallpaper.status === 'rejected' ? `
                            <button onclick="myUploads.editWallpaper(${wallpaper.id})" 
                                    class="text-green-600 hover:text-green-900">编辑</button>
                        ` : ''}
                        <button onclick="myUploads.deleteWallpaper(${wallpaper.id})" 
    ...
    renderPagination(pagination) {
        ...
        pageInfo.textContent = `显示 ${start}-${end} 共 ${pagination.total_items}`;
    ...
    async showDetail(wallpaperId) {
        try {
            const response = await fetch(`${this.API_BASE_URL}/my_user_uploads.php?action=detail&id=${wallpaperId}`, {
                credentials: 'include'
            });
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <div class="space-y-2">
                            ...
                            ${wallpaper.status === 'rejected' && wallpaper.reject_reason ? `
                                <div><span class="font-medium text-red-600">拒绝原因:</span> ${this.escapeHtml(wallpaper.reject_reason)}</div>
                            ` : ''}
                        </div>
                    </div>
    ...
    async deleteWallpaper(wallpaperId) {
        ...
        try {
            const response = await fetch(`${this.API_BASE_URL}/my_user_uploads.php`, {
                method: 'POST',
                headers: {
