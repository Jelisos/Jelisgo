...
<body class="font-inter bg-neutral text-dark min-h-screen flex flex-col">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // 从路径中提取壁纸ID
            function extractWallpaperIdFromPath(path) {
                // 匹配路径中的数字ID，如 static/wallpapers/001/ 或 static/preview/001/
                const match = path.match(/\/(\d{3})\//); 
                return match ? match[1] : null;
            }
            
            const wallpaperId = extractWallpaperIdFromPath(wallpaperPath);
            ...
            // 预览按钮点击事件
            document.getElementById('preview-wallpaper').onclick = async function() {
                if (wallpaperId) {
                    try {
                        // 使用Token化的原图URL
                        ...
                        window.open(`yulan.php?image=${encodeURIComponent(tokenizedUrl)}&id=${wallpaperId}#previewControls`, '_blank');
                    } catch (error) {
                        console.error('获取Token化URL失败:', error);
                        // 降级到原始路径，确保使用原图路径
                        let originalImagePath = wallpaperPath;
                        ...
                        if (originalImagePath.includes('static/preview/')) {
                            ...
                            console.log('[DetailMobile] 降级时转换预览图路径为原图路径:', originalImagePath);
                        }
                        
                        window.open(`yulan.php?image=${encodeURIComponent(originalImagePath)}&id=${wallpaperId}#previewControls`, '_blank');
                    }
                } else {
                    // 如果无法提取壁纸ID，使用原始路径
                    window.open(`yulan.php?image=${encodeURIComponent(wallpaperPath)}#previewControls`, '_blank');
                }
            };
