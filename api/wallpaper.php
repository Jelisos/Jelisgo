<?php
// 2025-01-27 完全弃用session功能，仅使用Authorization头认证
...
function handleGetWallpaperDetails() {
    ...
    ";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("iii", $wallpaperId, $userId, $userId);
...
function handleExileWallpaper($input_data) {
    ...
    // 2025-01-27 弃用session，仅使用Authorization头认证
    $userId = null;
    ...
    if (!$userId) {
        sendResponse(401, '未登录或缺少Authorization头');
        closeDBConnection($conn);
        return;
    }

    if (!isPermanentMemberOrAdmin($userId)) {
        sendResponse(403, '永久会员可操作');
        closeDBConnection($conn);
        return;
    }
    ...
    // 更新或插入流放状态，记录最后操作者用户ID
    $stmt = $conn->prepare("
        INSERT INTO wallpaper_exile_status (wallpaper_id, status, last_operator_user_id)
        VALUES (?, 1, ?)
        ON DUPLICATE KEY UPDATE status = 1, last_operator_user_id = ?;
    ");
    $stmt->bind_param("iii", $wallpaperId, $userId, $userId);
...
function handleRecallWallpaper($input_data) {
    ...
    // 更新流放状态为0 (召回)，记录最后操作者用户ID
    $stmt = $conn->prepare("UPDATE wallpaper_exile_status SET status = 0, last_operator_user_id = ? WHERE wallpaper_id = ?");
    $stmt->bind_param("ii", $userId, $wallpaperId);
...
function handleAdminBatchRecall() {
    ...
    try {
        ...
        // 批量更新 wallpaper_exile_status 表，记录最后操作者用户ID
        $stmtUpdate = $conn->prepare("UPDATE wallpaper_exile_status SET status = 0, last_operator_user_id = ? WHERE wallpaper_id IN ($placeholders)");
        $stmtUpdate->bind_param('i' . $types, $userId, ...$wallpaperIds);
        $stmtUpdate->execute();
...
function handleAdminBatchExile() {
    ...
    try {
        ...
        // 批量插入或更新 wallpaper_exile_status 表，记录最后操作者用户ID
        ...
        $affectedRows = 0;
        foreach ($wallpaperIds as $wallpaperId) {
            $stmtUpsert->bind_param("iii", $wallpaperId, $userId, $userId);
            $stmtUpsert->execute();
