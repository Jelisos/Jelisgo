...
error_reporting(E_ALL & ~E_NOTICE & ~E_WARNING); // 仅报告致命错误和解析错误
/**
 * 用户认证API
 * @author Claude
 * @date 2024-03-21
...
require_once 'email_validator.php'; // 邮箱域名验证工具
...
function handleRegister($data) {
    ...
    // 验证邮箱域名有效性（检查MX记录）
    if (!isEmailDomainValid($data['email'])) {
...
function handleLogin($data) {
    ...
    $sessionManager->setUserSession($user['id'], $userData);

    // 新增调试日志：记录会话ID和存储的用户ID
    sendDebugLog([
        'msg' => '登录成功，数据库session已设置',
