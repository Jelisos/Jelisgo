...
function logApiAccess($action, $params = [], $result = 'success') {
    // 增强错误处理：确保日志功能不会影响API正常运行
    try {
        $logDir = __DIR__ . '/../logs';
        $logFile = $logDir . '/wallpaper_api.log';
        
        // 检查父目录权限
        $parentDir = dirname($logDir);
        if (!is_dir($parentDir) || !is_writable($parentDir)) {
            return; // 静默跳过，不影响API功能
        }
    ...
    if (!$conn) {
        ...
        if (!$conn) {
            ...
            $debug_info = [
                ...
                'environment' => isOnlineEnvironment() ? 'online' : 'localhost'
            ];
